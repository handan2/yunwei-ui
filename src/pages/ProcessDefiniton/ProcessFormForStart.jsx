...
export default (props) => {
  ...
  useEffect(async () => {
    ...
    /*
     通过obj['计算机信息表.用户姓名'（template/label字段）]查找templateIdLableMap到对应的value就是控件中的name值,放在一个变量id中，再通过  core.setValue(id, xxxxx)来改变其值，但如果这个字段对应控件是一个select.怎么动态给这个select设置数据源呢，先得到找渲染出select字段的那个组件形式是啥，以及组件的名字是怎么命名的，todo断点
